...
export default function NewCasePage() {
  ...
  const [supportingDocuments, setSupportingDocuments] = useState<UploadedDocument[]>([])
  ...
  const canContinue = () => {
    ...
    if (isPrimaryFormStep && currentStepData?.documentType) {
      const isUploaded = primaryForms[currentStepData.documentType as PrimaryFormType] !== null
      const isOptional = currentStepData.optional === true
  ...
  const canSkip = () => {
    return isPrimaryFormStep && currentStepData?.optional === true
  }

  const isStepCompleted = () => {
    if (isPrimaryFormStep && currentStepData?.documentType) {
      return primaryForms[currentStepData.documentType as PrimaryFormType] !== null
    }
    return false
  }
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Follow the steps below to create a case and upload all required documents.
          </p>
        </div>
        ...
        <div className="min-h-[400px]">
          ...
          {isPrimaryFormStep && currentStepData?.documentType && (
            <DocumentUploadStep
              ...
              optional={currentStepData.optional}
            />
          )}
